...
//    }
    ...
    func getReportAPI(_ user_type: String, group_id: String, user_id: String, subgroup_id: String) {
        if Reachability.isConnectedToNetwork() {
            ...
            let param:[String:String] = [:]
            let url = "https://teamplayerhr.com/app-survey-result-team?group_id=\(group_id)&user_id=\(user_id)&subgroup_id=\(subgroup_id)&user_type=\(user_type)&token=\(UserDefaults.standard.value(forKey: USER_DEFAULTS_KEYS.VENDOR_SIGNUP_TOKEN)!)"
            ServerClass.sharedInstance.getRequestWithUrlParameters(param, path: url, successBlock: { (json) in
                print(json)
                hideAllProgressOnView(appDelegateInstance.window!)
                let success = json["success"].stringValue
...
//                self.getBrainTreeToken()
                ...
                let reportUrl = "https://teamplayerhr.com/app-survey-result-team?group_id=\(participantListObj.group_id)&user_id=\(participantListObj.user_id)&subgroup_id=\(participantListObj.subgroup_id)&user_type=participant&token=\(UserDefaults.standard.value(forKey: USER_DEFAULTS_KEYS.VENDOR_SIGNUP_TOKEN)!)"
        ...
        if Reachability.isConnectedToNetwork() {
            ...
            ServerClass.sharedInstance.postRequestWithUrlParameters(param, path: BASE_URL + PROJECT_URL.UPDATE_PPC_PAYMENT, successBlock: { (json) in
                ...
                if success == "true"
                {
                    ...
                    let reportUrl = "https://teamplayerhr.com/app-survey-result-team?group_id=\(self.selectedPpcPptObj.group_id)&user_id=\(self.selectedPpcPptObj.user_id)&subgroup_id=\(self.selectedPpcPptObj.subgroup_id)&user_type=benchmark&token=\(UserDefaults.standard.value(forKey: USER_DEFAULTS_KEYS.VENDOR_SIGNUP_TOKEN)!)"
